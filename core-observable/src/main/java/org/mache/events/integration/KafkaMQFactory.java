...
public class KafkaMQFactory<K, V extends CoordinationEntryEvent<K>> implements MQFactory<K, V> {
    ...
    {
        BaseCoordinationEntryEventProducer<K, V> producer = new KafkaEventProducer<K, V>(producerConfig, config.getTopicName());
        return producer;
    }
    ...
    {
        BaseCoordinationEntryEventConsumer<K, V> consumer = new KafkaEventConsumer<K, V>(consumerConfig, config.getTopicName());
        return consumer;
    }
